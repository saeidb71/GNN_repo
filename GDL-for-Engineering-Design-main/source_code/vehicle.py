...
for g in np.arange(4359):
    ...
    nxg=nx.Graph(A_i)
    btw_cent_g=list(nx.betweenness_centrality(nxg).values())
    eig_cent_g=list(nx.eigenvector_centrality(nxg).values())
    for node_i in np.arange(numNodes):
         node_feature_i[node_i][1]=btw_cent_g[node_i]
         node_feature_i[node_i][2]=eig_cent_g[node_i]
